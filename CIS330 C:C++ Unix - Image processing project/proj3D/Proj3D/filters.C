...
void Shrinker::Execute()
{
	...
	for(int i = 0; i < image.getHeight(); i++){
		for(int j = 0; j < image.getWidth(); j++){
			...
			image.getBuffer()[index] = input1->getBuffer()[oIndex];
			image.getBuffer()[index + 1] = input1->getBuffer()[oIndex + 1];
			image.getBuffer()[index + 2] = input1->getBuffer()[oIndex + 2];
		}
	}
}
	...
	for(int i = 0; i < image.getHeight(); i++){
		for(int j = 0; j < image.getWidth(); j++){
			...
			if(j < input1->getWidth()){
                ...
                image.getBuffer()[index] = input2->getBuffer()[oIndex];
                image.getBuffer()[index + 1] = input2->getBuffer()[oIndex + 1];
                image.getBuffer()[index + 2] = input2->getBuffer()[oIndex + 2];
			}
		}
	}
}
...
void Blender::Execute()
{
	...
	for(int i = 0; i < image.getHeight(); i++){
		for(int j = 0; j < image.getWidth(); j++){
            ...
            image.getBuffer()[index] = input1->getBuffer()[oIndex]*(percent) + input2->getBuffer()[oIndex] * (1 - percent);
            image.getBuffer()[index + 1] = input1->getBuffer()[oIndex + 1] * (percent) + input2->getBuffer()[oIndex + 1] * (1 - percent);
            image.getBuffer()[index + 2] = input1->getBuffer()[oIndex + 2] * (percent) + input2->getBuffer()[oIndex + 2] * (1 - percent);
		}
	}
}
